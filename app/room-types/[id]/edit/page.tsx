...
import { z } from 'zod';
import { updateRoomType } from '@/app/actions/roomTypes';
...
export default function EditRoomTypePage({ params }: { params: { id: string } }) {
    ...
    fetch(`/api/room-types/${params.id}`)
      ...
      .then((data) => {
        if (data.roomType) {
          ...
          // Check if room type is in predefined options
          if (roomTypeOptions.includes(roomTypeName)) {
            setSelectedRoomType(roomTypeName);
            setCustomRoomType('');
          } else {
  ...
  const onSubmit = async (data: RoomTypeForm) => {
    ...
    try {
      ...
      const result = await updateRoomType(params.id, formData);
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] p-6">
      ...
      <div className="bg-[#1e293b] rounded-lg border border-[#334155] p-6">
        <h1 className="text-2xl font-bold text-white mb-2">Edit Room Type</h1>
        <p className="text-gray-400">Update room type information</p>
      </div>
